...
export default class SearchOnlyVMBase2<T, T2> extends CommonSearchVMBase {
  ...
  CheckCriteriaEquals(searchCriteria: T2) {
    if (this.SearchCriteria == null) return false;
    return (
      JSON.stringify(this.SearchCriteria) == JSON.stringify(searchCriteria)
    );
  }
