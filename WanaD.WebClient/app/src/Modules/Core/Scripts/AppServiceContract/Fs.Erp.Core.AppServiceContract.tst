${
    ...
    string RenameNamespaceToModule(string name)
    {
        ...
        if(module.StartsWith("AllPay"))
        {
            int startIndex = module.IndexOf("AllPay") + 7;
    ...
    string CheckNameSpace(string name){
        ...
        else if(name.StartsWith("AllPay")){
            int startIndex = name.IndexOf("AllPay") + 7;
            string cut1 = name.Substring(startIndex);
            int endLength = cut1.IndexOf(".");
            string moduleName = "";
            string leftNamespace = "";
...
$Classes(c => c.Namespace == "Fs.Erp.Core.AppServiceContract")[
...
$Interfaces(c => c.Namespace == "Fs.Erp.Core.AppServiceContract")[
 ...
 $CalculatedModelTypes2[
    ...
    constructor(serverBaseUrl : string = null, controllerPath : string = null, ajaxOptions : JQueryAjaxSettings = null){
        this.ControllerUrl = ((serverBaseUrl == null) ? fs.Config.ServiceBaseUrl : serverBaseUrl) + ((controllerPath == null) ?  "$CheckNameSpaceController/$ServiceName" : controllerPath);
